...
def db_contacts_by_company(company: str, limit: int = 50):
    sql = """
        SELECT TOP (?)
            [First Name], [Last Name], [Job Title], [Office Email], [Office Name], [Industry], [PublicLink]
        FROM dbo.BusinessCards
        WHERE LOWER(LTRIM(RTRIM([Office Name]))) = LOWER(?)
        ORDER BY [Last Name], [First Name];
    """
...
def db_contacts_by_industry(industry: str, limit: int = 50):
    sql = """
        SELECT TOP (?)
            [First Name], [Last Name], [Job Title], [Office Name], [Office Email], [Industry], [PublicLink]
        FROM dbo.BusinessCards
        WHERE LOWER(LTRIM(RTRIM([Industry]))) = LOWER(?)
        ORDER BY [Last Name], [First Name];
    """
...
def extract_industry(message: str):
    ...
    # "list of names in healthcare"
    # "names under healthcare industry"
    # "companies in technology"
    patterns = [
        r"(?:people|contacts|names|name|list|list of names|list names)\s+(?:in|under|from)\s+([a-zA-Z &/-]+)$",
        r"(?:companies|company|list companies|list of companies)\s+(?:in|under|from)\s+([a-zA-Z &/-]+)$",
        r"(?:industry)\s*(?:is|=|:)?\s*([a-zA-Z &/-]+)$",
        r"(?:in|under|for|category)\s+([a-zA-Z &/-]+)$",
    ]
...
def extract_company(message: str):
    t = norm(message)
    # supports: "contacts from X", "people at X", "show X contacts"
    m = re.search(r"(?:from|at)\s+(.+)$", t)
    if m:
...
JOB_STOPWORDS = {
    "who","are","the","all","list","show","me","find","give","people","contacts","names","name",
    "in","under","from","at","of","for","with","role","position","job","title"
}

def extract_job_keyword(message: str):
    t = norm(message)

    # Capture after "who are" / "list" / "show"
    m = re.search(r"(?:who\s+are\s+|list\s+all\s+|list\s+|show\s+)(.+)$", t)
    ...
    # allow up to 3 words: "biomedical engineer", "sales manager"
    return " ".join(words[:3]).strip()

# =========================
# BERT ZERO-SHOT (INTENTS)
# =========================
ZSC_MODEL = os.getenv("ZSC_MODEL", "typeform/distilbert-base-uncased-mnli")
zsc = pipeline("zero-shot-classification", model=ZSC_MODEL)
...
def classify_intent(user_msg: str):
    ...
    # hard rules FIRST (prevents dumb mistakes like treating "hi" as an industry)
    ...
    # Strong heuristic: if user clearly asks "people/names in <industry>" → force contacts_by_industry
    if contains_any(t, ["people in ", "people under ", "contacts in ", "contacts under ", "names in ", "names under ", "list names in ", "list of names in "]):
        if extract_industry(user_msg):
            return "contacts_by_industry", 1.0

    # Strong heuristic: if user clearly asks companies in <industry> → force companies_by_industry
    if contains_any(t, ["companies in ", "companies under ", "company in ", "company under ", "list companies in ", "list of companies in "]):
        if extract_industry(user_msg):
            return "companies_by_industry", 1.0

    # person field heuristics
    if "email" in t:
        return "person_email", 0.95
    if contains_any(t, ["job title", "role", "position"]):
        return "person_job_title", 0.95
    if contains_any(t, ["phone", "number", "contact number"]):
        return "person_phone", 0.95
    if "industry" in t and contains_any(t, [" of ", " for ", "what is", "which industry"]):
        return "person_industry", 0.95
...
def api_chat():
    try:
        ...
        # If model is unsure AND no heuristic already forced it, ask user to rephrase
        if score < 0.50:
            return jsonify({
                "reply": (
        ...
        # Greeting
        if intent == "greeting":
            return jsonify({"reply": "Hey 🙂 Ask me things like: 'people in Healthcare' or 'companies in Technology'."})
        ...
        if intent == "contacts_by_company":
            ...
            if not company:
                return jsonify({"reply": "Which company? Example: 'contacts from Megapixel'."})
            ...
            lines = []
            for fn, ln, jt, email, comp, ind, pub in rows[:20]:
                lines.append(f"- {fn} {ln} — {jt or 'No job'} ({email or 'No email'})")
        ...
        # Contacts by industry (this is the one you care about)
        if intent == "contacts_by_industry":
            industry = extract_industry(user_msg)
            if not industry:
                return jsonify({"reply": "Which industry? Example: 'people in Healthcare'."})

            rows = db_contacts_by_industry(industry, limit=200)  # raise limit if you want more names
            if not rows:
                return jsonify({"reply": f"No contacts found in '{industry}'."})

            lines = []
            for fn, ln, jt, comp, email, ind, pub in rows[:20]:
                lines.append(f"- {fn} {ln} — {jt or 'No job'} ({comp or 'No company'})")

            reply = f"{len(rows)} contacts in {industry}:\n" + "\n".join(lines)
            if len(rows) > 20:
                reply += "\n\n(Showing first 20. Ask 'show more' if you add pagination later.)"
            return jsonify({"reply": reply})

        # People by job keyword (engineers, biomedical engineer, sales manager, etc.)
        if intent == "people_by_job_keyword":
            keyword = extract_job_keyword(user_msg)
            if not keyword:
                return jsonify({"reply": "Which role? Example: 'who are engineers' or 'list sales managers'."})

            rows = db_people_with_job_keyword(keyword, limit=200)
            if not rows:
                # try singular version too
                rows = db_people_with_job_keyword(singularize(keyword), limit=200)
            ...
            reply = f"{len(rows)} matches for '{keyword}':\n" + "\n".join(lines)
            if len(rows) > 20:
                reply += "\n\n(Showing first 20.)"
            return jsonify({"reply": reply})

        # Person field lookups
        if intent in ("person_email", "person_job_title", "person_phone", "person_industry"):
            first, last = split_name(user_msg)
            if not first or not last:
                return jsonify({"reply": "Who’s the person? Example: 'email of Rachel Sim'."})
        ...
        if intent == "search_name":
            ...
            rows = db_search_name(term, limit=200)
            if not rows:
                return jsonify({"reply": f"No contacts found for '{term}'."})
        ...
        if intent == "missing_fields":
            ...
            if "qr" in t:
                rows = db_missing("qrlink")
                return jsonify({"reply": f"{len(rows)} contacts are missing QR link."})

            return jsonify({"reply": "Missing what? Try: missing email / missing public link / missing phone / missing qr"})
...
if __name__ == "__main__":
    # IMPORTANT on Windows: prevents WinError 10038 (debug reloader socket issue)
